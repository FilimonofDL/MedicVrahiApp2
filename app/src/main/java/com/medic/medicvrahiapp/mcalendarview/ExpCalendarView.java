...
//            this.setId(R.id.calendarViewPager);                   не ясно зачем и где ид должно быть
        }
...
//        this.addOnPageChangeListener(new );
    ...
    public ExpCalendarView travelTo(DateData dateData) {
        ...
        MarkStyle markStyleToday = new MarkStyle();
        markStyleToday.setColor( getResources().getColor(R.color.colorPrimary));
        markStyleToday.setStyle(MarkStyle.BACKGROUND_CIRCLE_EMPTY);

        DateData todayMarkDate = dateData.setMarkStyle(markStyleToday);
//        calZapis.markDate(todayMarkDate);

//        MarkedDates.getInstance().add(todayMarkDate);
